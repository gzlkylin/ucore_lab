...
struct mm_struct *
mm_create(void) {
    struct mm_struct *mm = kmalloc(sizeof(struct mm_struct));//return the page pos virtual addr
...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    pte_t *ptep=NULL;
    /*LAB3 EXERCISE 1: xuetang gzlkylin@gmail.com
    ...
    */
    ...
    if (*ptep == 0) {
		pgdir_alloc_page(mm->pgdir, addr, perm);  //(2)
    }
    else {
    ...
    */
        if(swap_init_ok) {
            ...
            swap_map_swappable(mm, page, addr, 1);                        //(3) make the page swappable.
        }
        else {
